    ...
    function form($instance)
    {
        ...
        <p class="description">
            <label for="<?php echo $this->get_field_id('image_link') ?>">
                Image Link (If lightbox false)
                <?php echo aq_field_input('image_link', $block_id, $image_link) ?>
            </label>
        </p>
        <p class="description">
            <label for="<?php echo $this->get_field_id('image_link_target') ?>">
                Image Link Target
                <?php echo aq_field_select('image_link_target', $block_id, array('_self', '_blank'), $image_link_target) ?>
            </label>
        </p>
        <p class="description">
        ...
        <div class="row-fluid infobox">
            <div class="span12">
                <div id="<?php echo $widgetID ?>" class="met_img_with_text clearfix">
                    ...
                    <div class="met_img_with_text_preview">
                        ...
                        <?php if ($image_lightbox == 'true'): ?>
                            <div class="met_img_with_text_overlay met_bgcolor5_trans">
                                <a href="<?php echo $lbImage ?>" rel="lb_<?php echo $widgetID ?>"
                                   class="met_portfolio_item_lb met_bgcolor5 met_color2"><i class="icon-search"></i></a>
                            </div>
                        <?php else: ?>
                            <div class="met_img_with_text_overlay met_bgcolor5_trans">
                                <a href="<?php echo $image_link ?>" target="<?php echo $image_link_target ?>"
                    ...
                    <article class="met_color4 met_bgcolor5">
                        <div>
                            <?php if (!empty($title)): ?>
                                <h2><?php echo $title ?></h2>
                            <?php endif; ?>
                            <?php if (!empty($title_sub)): ?>
                                <h3 class="met_title_stack met_bold_one"><?php echo $title_sub ?></h3><br>
                            <?php endif; ?>
                            <?php if (!empty($text)): ?>
                                <div class="infobox_text"><?php echo htmlspecialchars_decode($text) ?></div>
                            <?php endif; ?>
                        </div>
                    </article>
                </div>
