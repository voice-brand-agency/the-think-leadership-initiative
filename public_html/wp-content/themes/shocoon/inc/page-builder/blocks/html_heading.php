...
if(!class_exists('MET_Heading_Block')) {
	class MET_Heading_Block extends AQ_Block {
		...
		function form($instance) {
			...
			<p class="description">
				<label for="<?php echo $this->get_field_id('ex_class') ?>">
					Extra Class (You can input multiple class with space)<br/>
					<?php echo aq_field_input('ex_class', $block_id, $ex_class) ?>
				</label>
			</p>
		<?php
		...
		function block($instance) {
			extract($instance);
			echo '<'.$type.' class="'.(($weight == 1) ? 'met_bold_one' : '').' '.((isset($ex_class)) ? $ex_class : '').'">' . htmlspecialchars_decode($title) . '</'.$type.'>';
		}
