...
if(!class_exists('MET_Tabs_Block')) {
	class MET_Tabs_Block extends AQ_Block {
		...
		function form($instance) {
			...
			<p class="description">
				<label for="<?php echo $this->get_field_id('selected_no') ?>">
					Selected Item No<br/>
					<?php echo aq_field_input('selected_no', $block_id, $selected_no, 'min', 'number') ?>
				</label>
			</p>
		<?php
		}
...
?>
				<ul id="myTab" class="nav nav-tabs met_tab_nav met_bgcolor">
					<?php
					$i = 1;
					if(!isset($selected_no)){ $selected_no = $i; }
					foreach( $tabs as $tab ): ?>
						<li <?php echo (($i == $selected_no) ? 'class="active"' : '') ?>><a href="#<?php echo sanitize_title( $tab['title'] ) . $i ?>" data-toggle="tab"><?php echo $tab['title'] ?></a></li>
					<?php $i++; endforeach; ?>
				</ul>
				<div id="<?php echo $widgetID ?>" class="tab-content met_tab_content">
				<?php $i = 1; foreach( $tabs as $tab ): ?>
					<div class="tab-pane fade <?php echo (($i == $selected_no) ? 'active in' : '') ?>" id="<?php echo sanitize_title( $tab['title'] ) . $i ?>">
...
<?php
				...
				foreach( $tabs as $tab ){
					...
					$open = $i == $selected_no ? 'in' : 'collapsed';
					...
					<div class="accordion-group scrolled__item">
						<div class="accordion-heading">
							<a class="accordion-toggle '.$open.'" data-toggle="collapse" data-parent="#'.$widgetID.'" href="#collapse_'.$i.'">
								<span class="met_toggle_text">'. $tab['title'] .'</span>
								<span class="met_icon_'.$minusPlus.'"></span>
							</a>
						</div>
						<div id="collapse_'.$i.'" class="accordion-body collapse '.$open.'">
